...
3. **每节讲完出 5 道选择题**（PMP 风格：情景题干 + 选项 A-D，可单选/多选）。**出题时先不给答案**，等用户作答后再判分 + 逐题解析 + 揭示答案。用户答对/理解才进下一节。
   - **出题不暴露答案（必须遵守）**：
     ...
     - ④ **优先从 `04-practice` 抽真题**：原创仿造题须自检①②③；条件允许时优先抽题库现成题，从源头规避"随手举例即暴露答案"。
...
**精简示例**（错因只留"判别关键+次数"，概念正文已在 concepts/recurring_patterns 不复述；答案可并入题干）：

~~~
## MISTAKE -> 05-mistakes/process
- 日期：2026-07-27
- 题目：业务重组改流程/作废设计，团队重排待办转新方向交付。我选A韧性
- 正解：B适应性
- 错因：换方向非恢复原轨->适应性。第2次
- 关联：适应性vs韧性(2次)
~~~
